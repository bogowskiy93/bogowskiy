...
function buildFloatField() {
  ...
  projects.forEach((project, idx) => {
    const [x, y] = slots[idx % slots.length];
    const size = isMobile
      ? 88 + ((idx * 29) % 48)
      : 110 + ((idx * 37) % 90);
    const dur = 22 + ((idx * 13) % 18);
    const delay = -((idx * 1.7) % 20);
    const op = 0.28 + ((idx * 17) % 22) / 100;
    const anim = anims[idx % anims.length];
    const jx = ((idx * 19) % 11) - 5;
    const jy = ((idx * 23) % 9) - 4;
...
function buildCard(project, idx) {
  ...
  const order = idx;
  card.style.setProperty('--enter-delay', `${80 + order * 40}ms`);
